...
function SharePanel({ sessionData }) {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-primary-dark to-primary-darker rounded-lg p-6 shadow-xl border border-primary-600 text-center"
    >
      ...
      <div className="flex items-center justify-between bg-surface rounded-lg p-3 pr-4 mb-6 break-all">
        <span className="text-gray-300 text-sm overflow-hidden text-ellipsis whitespace-nowrap">{sessionData.shareableLink}</span>
        <button
          onClick={handleCopyLink}
          className="ml-4 px-4 py-2 bg-accent text-white rounded-md hover:bg-accent-dark transition-colors flex-shrink-0"
        >
          Copy Link
        </button>
      </div>
